...
package edu.wit.scds.ds.common.chains.enhanced ;

import static edu.wit.scds.ds.common.utilities.VisualizationSupport.DATA_REFERENCE_MARKER ;
import static edu.wit.scds.ds.common.utilities.VisualizationSupport.FIELD_SEPARATOR ;
import static edu.wit.scds.ds.common.utilities.VisualizationSupport.NEXT_REFERENCE_MARKER ;
import static edu.wit.scds.ds.common.utilities.VisualizationSupport.NULL_REFERENCE_MARKER ;
import static edu.wit.scds.ds.common.utilities.VisualizationSupport.PSEUDO_ADDRESS_PREFIX ;
...
/**
 * Class {@code Node} provides the basis for singly-linked chain
 * functionality<br>
 * based on material from Carrano and Henry's "Data Structures and Abstractions
 * with Java", 4th edition and from the JCL
 * <p>
 * Note: This enhanced implementation includes display methods for educational
 * purposes.
 * </p>
 *
 ...
 *     </ul>
 * @version 4.11 2021-09-27 enhance {@code toString()} to handle {@code null}
 *     {@code data}
 ...
 *     <li>revise {@code toString()} to reduce space and better resemble an
 *     in-class diagram of a {@code Node}
 *     </ul>
 * @version 4.14 2022-09-29 minor cosmetic changes
 * @version 4.15 2022-10-30
 *     <ul>
 *     <li>minor cosmetic changes
 *     <li>change arrow to {@code data} references to better distinguish from
 *     {@code next} references
 *     </ul>
 ...
 *     <li>move expanded set of symbolic constants for debugging, etc. display
 *     to separate class
 *     </ul>
 * @version 5.1 2024-09-17
 *     <ul>
 *     <li>switch from local constants to {@code VisualizationSupport}
 *     <li>add data and next reference markers even if null to clarify which is
 *     which
 *     </ul>
 * @version 5.1 2024-10-06 replace literal right arrow (next reference) with
 *     symbolic equivalent in {@code chainToString()}
 ...
 *     <li>rename the parameter for the 1-arg constructor which takes a data
 *     reference to be consistent with naming convention
 *     </ul>
 ...
 *     </ul>
 * @version 7.1 2025-03-19 add {@code DEFAULT_INITIAL_PSEUDO_ADDRESS},
 *     {@code resetNextPseudoAddress()} and {@code setNextPseudoAddress()} to
 *     support unit testing so the ids in corresponding {@code Node}s in user's
 *     chain match those in the expected/correct chain
 ...
 *     </ul>
 * @version 8.1 2025-09-27 add various {@code toList()} and
 *     {@code toListOfNodes()}
 *
 * @param <T>
...
public final class Node<T>
    ...
    /**
     * Sets up a node: {@code data} and {@code next} pointers are set to
     * {@code null}.
     */
    public Node()
        {
    ...
    /**
     * Sets up a node given supplied {@code data}; {@code next} pointer is set
     * to {@code null}.
     *
     * @param initialData
    ...
    /**
     * Sets up a node given supplied {@code next}; {@code data} pointer is set
     * to {@code null}.
     *
     * @param initialNext
     ...
     * <p>
     * this supports detailed logging with application's addresses matching the
     * expected/correct {@code Node}s addresses
     *
     ...
     *     flag to include ({@code true}) or omit ({@code false}) each
     *     {@code Node}'s position starting with the first {@code Node} in the
     *     chain - each position is equivalent to the index in a corresponding
    ...
    /**
     * convenience method - generates a String representation of a chain of
     * {@code Node}s
     * <p>
     * this is a debugging aid
     *
     * @param firstNode
     *     the starting point for the chain, typically {@code this.firstNode}
     *     (or similar) from the collection
     *
     * @return a text representation of the chain of {@code Node}s
     */
     ...
     * this is a debugging aid
     *
     * @return the contents of the chain in the order in which they occur in the
     *     chain
     *
     ...
     *
     * @param contents
     *     will contain the contents of the chain in the order in which they
     *     occur in the chain, excluding the duplicate if a loop is detected
     *     <p>
     *     the list is cleared before (re-)populating it
     *
     * @return the number of {@code Node}s visited, includes the looping
     *     {@code Node} if loop is detected
     *
     * @throws IllegalStateException
     *     if a loop is detected in the chain
     ...
     *
     * @return the contents of the chain in the order in which they occur in the
     *     chain or {@code null} if {@code startNode} is {@code null}
     *
     * @throws IllegalStateException
     *     if a loop is detected in the chain
     *
     ...
     *
     * @return the number of {@code Node}s visited, includes the looping
     *     {@code Node} if loop is detected; -1 if {@code startNode} is
     *     {@code null}
     *
     * @throws IllegalStateException
     *     if a loop is detected in the chain
     ...
     * this is a debugging aid
     *
     * @return the {@code Node}s in the chain in the order in which they occur
     *     in the chain, excluding the duplicate if a loop is detected
     *
     * @throws IllegalStateException
     *     if a loop is detected in the chain
     *
     ...
     *
     * @param listOfNodes
     *     will contain the {@code Node}s in the chain in the order in which
     *     they occur in the chain, excluding the duplicate if a loop is
     *     detected
     *     <p>
     *     the list is cleared before (re-)populating it
     *
     ...
     * @since 8.1
     */
    public int toListOfNodes( final List<Node<T>> listOfNodes )
            throws IllegalStateException, NullPointerException
        {
     ...
     *
     * @return the {@code Node}s in the chain in the order in which they occur
     *     in the chain, excluding the duplicate if a loop is detected; or
     *     {@code null} if {@code startNode} is {@code null}
     *
     * @throws IllegalStateException
     *     if a loop is detected in the chain
     *
     * @since 8.1
     */
    public static <T> List<Node<T>> toListOfNodes( final Node<T> startNode )
            throws IllegalStateException
        {
     ...
     *     the list is cleared before (re-)populating it
     *
     * @return the number of {@code Node}s visited; or -1 if {@code startNode}
     *     is {@code null}
     *
     * @throws IllegalStateException
     *     if a loop is detected in the chain
    ...
    public static void main( final String[] args )
        ...
        // first element
        firstNode = new Node<>() ;
        final Node<String> nullNode = firstNode ;    // for convenience to
                                                     // create a loop later
        ...
        final Node<String> cNode = firstNode ;    // for convenience to create
                                                  // another loop later
